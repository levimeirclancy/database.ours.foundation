...
$style_array = [
	...
	"article, article th, article td, p, summary, blockquote" =>
		[
		"font-weight"		=> "400",
		"text-align"		=> "left",
		"font-family"		=> "Times, Serif",
...
"@media only screen and (max-width"			=> "500px) "
		...
		"article th, article td"
		[ display"			=> "block",}
		"article th + th, article td + td, article th + td, article td + th"
		[ border-left"			=> "none", border-top"			=> "2px solid rgba(200,200,200,0.12)", }
		],
...
function output_css ($array) {
	...
	// First, check 
	foreach ($array as $key_temp => $value_temp):
	
		echo $key_temp." {";
	
		foreach ($value_temp as $key_sub_temp => $value_sub_temp):
			if (!(is_array($value_sub_temp))): continue; endif;
			
			output_css($value_temp);
			continue 2;
			endforeach;
	
		foreach ($value_temp as $key_sub_temp => $value_sub_temp):
	
			echo $key_sub_temp .":". $value_sub_temp .";"
